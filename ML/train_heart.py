...
# 1. Fetch Dataset
log.info("Downloading UCI Heart Disease dataset (ID 45)...")
heart = fetch_ucirepo(id=45)
df = heart.data.features.copy()
df["target"] = (heart.data.targets > 0).astype(int)

# Target Features required by Backend
...
# We calculate SHAP on the preprocessed test set
...
try:
    if best_model_name in ["RandomForest", "GradientBoosting"]:
        ...
        # Random Forest returns list of arrays for binary class, GB returns array
        if isinstance(shap_values, list):
            shap_values = shap_values[1]
    else:
        # LinearExplainer needs independent background data or just use ExactExplainer
    ...
    feature_importance.to_csv(MODELS_DIR / "heart_shap_importance.csv", index=False)
except Exception as e:
    log.error("SHAP calculation failed: %s", e)

# 5. Export
joblib.dump(best_model.named_steps["clf"], MODELS_DIR / "heart_model.pkl")
joblib.dump(best_model[:-1], MODELS_DIR / "heart_scaler.pkl") # save the whole preprocessor as scaler
log.info("Saved: heart_model.pkl, heart_scaler.pkl")
