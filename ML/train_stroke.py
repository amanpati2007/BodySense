"""
ML Training — Stroke Risk
==========================
Dataset : Kaggle Stroke Prediction Dataset
Source  : GitHub Gist Mirror (Verified Kaggle CSV)
...
log = logging.getLogger("train_stroke")
...
CSV_PATH = DATASETS_DIR / "stroke.csv"
...
log.info("Downloading stroke dataset from verified mirror...")
try:
    urllib.request.urlretrieve(URL, CSV_PATH)
except Exception as e:
    log.error("Failed to download stroke dataset: %s", e)
    raise

df = pd.read_csv(CSV_PATH)

# Encode categoricals safely without data leakage (just mapping to integers)
...
df["smoking_status"] = df["smoking_status"].map(smoke_map).fillna(3)

# ensure bmi is float, unknown converted to nan for imputer
df["bmi"] = pd.to_numeric(df["bmi"], errors="coerce")

FEATURE_COLS = ["age", "hypertension", "heart_disease", "ever_married",
...
log.info("Starting Hyperparameter Optimization (with balanced class weights to avoid data leakage)...")

# Notice: class_weight='balanced' replaces the flawed upsampling that was causing data leakage in the previous version
pipelines = {
    ...
    # GradientBoosting doesn't support class_weight natively in sklearn out of the box in the same way, but it handles imbalance better, 
    # we'll exclude it here to prioritize class_weight models, or we can use XGBoost (if installed) or scale_pos_weight. 
    # We will stick to LR and RF which are very robust with class_weight.
}
...
try:
    ...
    feature_importance.to_csv(MODELS_DIR / "stroke_shap_importance.csv", index=False)
except Exception as e:
    log.error("SHAP calculation failed: %s", e)

# 5. Export
joblib.dump(best_model.named_steps["clf"], MODELS_DIR / "stroke_model.pkl")
joblib.dump(best_model[:-1], MODELS_DIR / "stroke_scaler.pkl")
log.info("Saved: stroke_model.pkl, stroke_scaler.pkl")
