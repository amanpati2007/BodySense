"""
ML Training — Parkinson's Disease
===================================
Dataset : UCI Parkinsons Dataset
Source  : UCI Machine Learning Repository (ID 174)
License : CC BY 4.0
Features: 22 voice measure features
...
log = logging.getLogger("train_parkinsons")

MODELS_DIR = Path(__file__).parent / "models"
MODELS_DIR.mkdir(exist_ok=True)

# 1. Fetch Dataset
log.info("Downloading UCI Parkinson's Disease dataset (ID 174)...")
parkinsons = fetch_ucirepo(id=174)
df = parkinsons.data.features.copy()
df["target"] = parkinsons.data.targets.copy()

# Fix DuplicateError from UCI ML repo (MDVP:Jitter and MDVP:Shimmer appear twice)
# Also align directly with backend names
FEATURE_COLS = [
...
try:
    ...
    feature_importance.to_csv(MODELS_DIR / "parkinsons_shap_importance.csv", index=False)
except Exception as e:
    log.error("SHAP calculation failed: %s", e)

# 5. Export
joblib.dump(best_model.named_steps["clf"], MODELS_DIR / "parkinsons_model.pkl")
joblib.dump(best_model[:-1], MODELS_DIR / "parkinsons_scaler.pkl")
log.info("Saved: parkinsons_model.pkl, parkinsons_scaler.pkl")
